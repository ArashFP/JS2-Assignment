...
function ProductDetailsPage() {
  ...
  useEffect(() => {
    ...
    const getProduct = async () => {
      ...
      setProduct(res.data);
      setSelectedImage(res.data.images[0]); // Image that is displayed first 
      setLoading(false);
    };
        ...
        <div className="inline-flex flex-col items-center border-2 border-gray-500 rounded-2xl shadow-[2px_2px_10px_black] p-8 bg-white mt-20">
          <div className="flex items-start">
            ...
            <div className="max-w-[20rem]">
              ...
              <button className='text-white border-2 px-5 py-1 rounded-2xl bg-blue-600 '> Add to Cart</button>
              </div>
            </div>
          </div>
